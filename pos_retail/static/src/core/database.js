...
odoo.define('pos_retail.database', function (require) {
    ...
    db.include({
        ...
        save_invoices: function (invoices) {
            this.set_last_write_date_by_model('account.move', invoices);
        ...
        save_invoice_lines: function (lines) {
            this.set_last_write_date_by_model('account.move.line', lines);
            for (var i = 0; i < lines.length; i++) {
                var line = lines[i];
                if (!line.move_id) {
                    continue
                ...
                var lines_invoice_by_id = this.lines_invoice_by_id[line.move_id[0]];
                if (lines_invoice_by_id) {
                    ...
                    this.lines_invoice_by_id[line.move_id[0]] = lines_invoice_by_id;
                } else {
                    this.lines_invoice_by_id[line.move_id[0]] = [line]
                }
            }
        },
