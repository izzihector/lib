odoo.define('pos_retail.restaurant', function (require) {
    ...
    models.load_models({
        ...
        loaded: function (self, floors) {
            ...
            for (var i = 0; i < floors.length; i++) {
                floors[i].tables = [];
                self.floor_by_id[floors[i].id] = floors[i];
    ...
    models.load_models({
        ...
        loaded: function (self, tables) {
            ...
            for (var i = 0; i < tables.length; i++) {
                var table = tables[i];
                self.table_by_id[table.id] = table;
                var floor = self.floor_by_id[table.floor_id[0]];
                if (floor) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        set_idle_timer(deactive, timeout) {
            if (this.client_set_table) {
                return false
            } else {
                return _super_posmodel.set_idle_timer.apply(this, arguments);
            }
        },
        set_table: function (table) {
            ...
            if (!this.pos_bus) {
                this.client_set_table = true;
                _super_posmodel.set_table.apply(this, arguments);
                this.client_set_table = false;
            } else {
        ...
        sync_order_adding: function (vals) {
            ...
            if (order_exist) {
                ...
                if (this.config.screen_type !== 'waiter') {
                    ...
                    if (vals['floor_id'] && !this.floor_by_id[vals['floor_id']]) {
                        vals['floor_id'] = null;
                    }
                    if (vals['table_id'] && !this.floor_by_id[vals['table_id']]) {
                        vals['table_id'] = null;
                    }
        ...
        sync_order_transfer_new_table: function (vals) {
            ...
            if (order != undefined) {
                if (this.floor_by_id[vals.floor_id] && this.table_by_id[vals.table_id]) {
                    var table = this.table_by_id[vals.table_id];
                    var floor = this.floor_by_id[vals.floor_id];
                    if (table && floor) {
                        order.table = table;
