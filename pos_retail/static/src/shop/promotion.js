            ...
            if (rule_applied && product_discount) {
                ...
                for (var product_id in rule_applied) {
                    ...
                    if (total_qty >= qty_apply) {
                        ...
                        promotion_reason = product['display_name'] + ': buy ' + (total_qty - qty_apply) + ' with price: ' + this.pos.gui.chrome.format_currency(product['lst_price']) + '. And ' + qty_apply + ' with price ' + this.pos.gui.chrome.format_currency(rule_applied[product_id]['lst_price']);
                        if (price_discount < 0) {
