    ...
    var products_operation = screens.ScreenWidget.extend({ // products screen
        ...
        show: function () {
            ...
            this.$('.sort_by_product_operation_list_price').click(function () {
                var products = self.pos.db.get_product_by_category(0).sort(self.pos.sort_by('lst_price', self.reverse, parseInt));
                self.render_list(products);
                self.reverse = !self.reverse;
            });
        ...
        product_icon_url: function (id) {
            return '/web/image?model=product.product&id=' + id + '&field=image_128';
        },
        save_product_edit: function (product) {
            ...
            fields['lst_price'] = parseFloat(fields['lst_price']);
            ...
            if (fields['id']) {
                rpc.query({
                    ...
                    .then(function (result) {
                        self.pos.get_modifiers_backend('product.product').then(function () {
                            self.pos.gui.show_popup('dialog', {
                                title: 'Saved',
                                body: 'Product saved',
                                color: 'success'
