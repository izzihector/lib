odoo.define('pos_retail.buttons', function (require) {
        ...
        button_click: function () {
            ...
            this.pos.gui.show_popup('popup_dynamic_combo', {
                ...
                confirm: function (selected_combo_items) {
                    ...
                    selected_line.selected_combo_items = selected_combo_items;
                    if (price_extra) {
                        selected_line.set_unit_price(selected_line.product.lst_price + price_extra);
                    } else {
        ...
        button_click: function () {
            ...
            if (!client) {
                ...
                this.pos.gui.show_popup('dialog', {
                    title: 'Warning',
                    body: _t('Could not show Purchased Histories Products, required choice Client the first')
                })
            } else {
                ...
                if (!orders) {
                    this.pos.gui.show_popup('dialog', {
                        title: 'Warning',
                        body: _t('Client never Purchase from your Shop')
                    })
                } else {
                    return rpc.query({
    ...
    var button_global_discount_value = screens.ActionButtonWidget.extend({ // global discounts
        template: 'button_global_discount_value',
        button_click: function () {
            var self = this;
            this.gui.show_popup('number', {
                title: _t('Global Discount Value'),
                body: 'Maximum value you can set is ' + this.pos.gui.chrome.format_currency(this.pos.discount_value_limit),
                confirm: function (discount_value) {
                    var order = self.pos.get_order();
                    if (order) {
    ...
    screens.define_action_button({
        'name': 'button_global_discount_value',
        'widget': button_global_discount_value,
        'condition': function () {
            return this.pos.config.discount_value && this.pos.config.discount_value_limit > 0;
        }
    });
    ...
    var button_signature_order = screens.ActionButtonWidget.extend({
        template: 'button_signature_order',
        button_click: function () {
            var order = this.pos.get_order();
