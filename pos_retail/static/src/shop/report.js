odoo.define('pos_retail.report', function (require) {
    ...
    var report = screens.ScreenWidget.extend({
        ...
        print_xml: function () {
            ...
            if (this.pos.epson_printer_default && this.pos.report_xml && this.pos.gui) {
                ...
                var i = 0;
                while (i < this.pos.no_of_copies) {
                    this.pos.proxy.printer.print_receipt(this.pos.report_xml);
                    i++;
                }
            }
        },
    ...
    var popup_report_product_summary = PopupWidget.extend({
        ...
        click_confirm: function () {
            ...
            if (fields.current_session_report) {
                ...
                this.from_date = null;
                this.to_date = null;
                return rpc.query(params, {async: false}).then(function (results) {
                    if (results) {
        ...
        render_report: function (results) {
            ...
            if (results) {
                if (Object.keys(results['category_report']).length == 0 && Object.keys(results['order_report']).length == 0 &&
                    ...
                    if (results['category_report']) {
                        if (state) {
                            _.each(results['category_report'], function (value, key) {
                                total_categ_amount += value[1];
                            });
                        }
                    }
                    if (results['payment_report']) {
                        if (state) {
                            _.each(results['payment_report'], function (value, key) {
                                total_amount += value;
                            });
                        }
                    }
                ...
                return rpc.query(params, {async: false}).then(function (res) {
                    if (res) {
                        if (Object.keys(res['journal_details']).length == 0 && Object.keys(res['salesmen_details']).length == 0) {
                            ...
                            self.pos.report_html = QWeb.render('payment_summary_receipt', {
                                widget: self,
                                pos: self.pos,
                                journal_details: journal_details,
                                salesmen_details: salesmen_details,
    ...
    var button_reporting = screens.ActionButtonWidget.extend({
        template: 'button_reporting',
        button_click: function () {
            var self = this;
            var list_report = [];
            if (this.pos.config.report_product_summary) {
                list_report.push({
                    'label': 'Report Products Summary',
                    'item': 1
                })
            }
            if (this.pos.config.report_order_summary) {
                list_report.push({
                    'label': 'Report Orders Summary',
                    'item': 2
                })
            }
            if (this.pos.config.report_payment_summary) {
                list_report.push({
    ...
    screens.define_action_button({
        'name': 'button_reporting',
        'widget': button_reporting,
        'condition': function () {
            return !this.pos.pos_session.mobile_responsive && (this.pos.config.report_product_summary || this.pos.config.report_order_summary || this.pos.config.report_payment_summary || this.pos.config.report_sale_summary);
        }
    });
});
