...
odoo.define('pos_retail.screen_order_widget', function (require) {
    ...
    screens.ProductCategoriesWidget.include({
        renderElement: function () {
            this._super();
            this.pos.gui.remove_keyboard();
            this.pos.gui.add_keyboard('products');
        }
    });
    screens.OrderWidget.include({
            ...
            this.keyboard_handler = function (event) {
                ...
                if (event.type === "keypress") {
                    if (event.keyCode === 32) { // Space
                        ...
                        $('.show_hide_pad').click();
                    } else if (event.keyCode === 102 && self.pos.config.quickly_payment_full && self.pos.config.quickly_payment_full_method_id) { // key: f
                        $('.quickly_paid').click();
        ...
        renderElement: function (scrollbottom) {
            ...
            setTimeout(function () {
                ...
                $('.order_signature').click(function () {
                    var order = self.pos.get_order();
                    if (order) {
        ...
        event_input_linked_keyboard_event: function () {
            ...
            //          -----------------------------------------------------------------
            var self = this;
            if (this.getParent().el.querySelector('.searchbox >input')) {
                this.getParent().el.querySelector('.searchbox >input').addEventListener('focus', function () {
                    self.remove_event_keyboard();
                });
                this.getParent().el.querySelector('.searchbox >input').addEventListener('blur', function () {
                    self.remove_event_keyboard();
                    self.add_event_keyboard();
                });
            }
            if (this.getParent().el.querySelector('.find_partner_input')) {
            ...
            if (selected_order && buttons) {
                ...
                */
                ...
                for (var i=0; i < selected_order.orderlines.models.length; i++) {
                    total_items += selected_order.orderlines.models[i].quantity;
                }
                this.set_total_items(total_items);
