...
odoo.define('pos_retail.screen_sale_orders', function (require) {
    ...
    models.load_models(
        {
            ...
            loaded: function (self, account_payment_methods) {
                self.account_payment_methods = account_payment_methods;
                self.account_payment_method_by_id = {};
                for (var i = 0; i < account_payment_methods.length; i++) {
                    var account_payment_method = account_payment_methods[i];
    ...
    var popup_create_booking_order = PopupWidget.extend({ // create booking order
        ...
        renderElement: function () {
            var self = this;
            this._super();
            this.$('.create_booked_order').click(function () {
                var fields = {};
                ...
                var payment_partial_journal_id = parseInt(fields.payment_partial_journal_id);
                ...
                if (!payment_partial_journal_id && payment_partial_amount > 0) {
                    return self.wrong_input('input[id="payment_partial_journal_id"]', '(*) Journal is required');
                } else {
                    self.passed_input('input[id="payment_partial_journal_id"]');
                }
                var so_val = order.export_as_JSON();
                var value = {
                ...
                if (payment_partial_amount > 0) {
                    var payment = {
                        ...
                        payment_method_id: payment_partial_method_id
                    };
        ...
        init: function (parent, options) {
            ...
            this.pos.bind('new:booking_order', function (order_id) {
                ...
                return self.pos.gui.show_popup('confirm', {
                    title: 'New',
                    body: 'Have one booking order send to you, are you want open it ?',
                    confirm: function () {
            ...
            this.$('.action_covert_to_pos_order').click(function () {
                ...
                if (self.sale_selected) {
                    ...
                    if (self.sale_selected['payment_partial_amount']) {
                        self.pos.gui.show_popup('dialog', {
                            title: 'Warning',
                            body: 'This order have partial payment amount: ' + self.sale_selected['payment_partial_amount'],
                            color: 'success'
                        });
                        ...
                        var payment_method = _.find(self.pos.payment_methods, function (method) {
                            return method.journal && method.journal['id'] == payment_partial_journal_id;
                        });
                        if (payment_method) {
