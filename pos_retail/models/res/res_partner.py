...
class res_partner(models.Model):
    ...
    wallet = fields.Float(digits=(16, 4),
                          compute='_compute_wallet', string='Wallet Amount',
    ...
    pos_loyalty_point_import = fields.Float('Loyalty Points Import', default=0,
                                            help='Admin system can import point for this customer')
    pos_loyalty_point = fields.Float(compute="_get_point", string='Loyalty Points',
    ...
    group_ids = fields.Many2many('res.partner.group', 'res_partner_group_rel', 'partner_id', 'group_id',
                                 string='Groups Name')
    ...
    def _compute_wallet(self):
        for partner in self:
            ...
            payment_datas = self.env.cr.fetchall()
            if len(payment_datas) == 1 and payment_datas[0] and payment_datas[0][0]:
                partner.wallet = - (payment_datas[0][0])
