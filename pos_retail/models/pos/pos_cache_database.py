...
class PosCacheConfig(models.Model):
    ...
    _description = "Cache Master Data for POS"
    _rec_name = "config_id"
    ...
    json_datas = fields.Text('Json Datas')

    def save_cache_to_config(self, vals):
        caches = self.search([('config_id', '=', vals.get('config_id'))])
        if not caches:
            return self.create({
                'config_id': vals.get('config_id'),
                'json_datas': json.dumps(vals.get('json_datas'))
            }).id
        else:
            return caches[0].id
...
class PosCacheDatabase(models.Model):
    ...
    def send_notification_pos(self, model_name, id, deleted=None):
    ...
    def get_data_by_ids(self, model, ids):
        results = []
        for id in ids:
            value = {
                'model': model,
    ...
    def remove_record(self, model, record_id):
        ...
        if records:
            ...
            self.create(vals)
        self.send_notification_pos(model, record_id, True)
        return True
